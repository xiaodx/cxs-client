...
//	char[] CA = "efgQRSTUabIJk560178lmnopqr23cdVWXYFstABCuvwxyzGH49!.ZhijKLMNOPDE".toCharArray();
	int[] IA = new int[256];
	...
	public static final byte[] encodeToByte(byte[] b) {
		Base64 encoder = null;
		try {
			encoder = Base64Cache.getBase64();
			return encoder.encodeByte(b, false);
		} finally {
			Base64Cache.resetBase64(encoder);
		}
	}

	public static final byte[] encodeToByte(String table, byte[] b) {
	 ...
	 * @return A BASE64 encoded array. Never <code>null</code>.
	 */
	public final byte[] encodeByte(byte[] sArr, boolean lineSep) {
		// Check special case
		int sLen = sArr != null ? sArr.length : 0;
		if (sLen == 0)
			return new byte[0];
	...
	public static final byte[] decodeFromByte(byte[] b) {
		Base64 encoder = null;
		try {
			encoder = Base64Cache.getBase64();
			byte buf[] = encoder.decodeByte(b);
			return buf;
		} finally {
			Base64Cache.resetBase64(encoder);
		}
	...
	public static final byte[] decodeFromByte(String table, byte[] b) {
		Base64 encoder = new Base64(table);
		return encoder.decodeByte(b);
	}
	 ...
	 *         divideable by 4. (I.e. definitely corrupted).
	 */
	public final byte[] decodeByte(byte[] sArr) {
		// Check special case
		int sLen = sArr.length;
	...
	public static String decode(String s) throws UnsupportedEncodingException {
		Base64 decoder = null;
		try {
			decoder = Base64Cache.getBase64();
			return new String(decoder.decodeFast(s), "UTF-8");
		} finally {
			Base64Cache.resetBase64(decoder);
		}
	}

	public static String decode(String s, String table)
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		...
		String jdk = MD5Utils.md5(java.util.Base64.getEncoder().encodeToString(input.getBytes("utf-8")));
		String cur = MD5Utils.md5(Base64.encode(input));
